...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    wh = jt.array(np.concatenate([l[:, 3:5] * s for s, l in zip(shapes * scale, dataset.labels)])).float()  # wh
    ...
    bpr, aat = metric(m.anchor_grid.clone().view(-1, 2))
    ...
    if bpr < 0.98:  # threshold to recompute
        ...
        if new_bpr > bpr:  # replace anchors
            new_anchors = jt.array(new_anchors).type_as(m.anchors)
            m.anchor_grid[:] = new_anchors.clone().view_as(m.anchor_grid)  # for inference
            m.anchors[:] = new_anchors.clone().view_as(m.anchors) / m.stride.view(-1, 1, 1)  # loss
